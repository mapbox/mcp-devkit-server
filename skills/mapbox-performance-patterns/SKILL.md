---
name: mapbox-performance-patterns
description: Performance optimization patterns for Mapbox applications. Covers initialization waterfalls, bundle size, rendering performance, memory management, and mobile optimization. Prioritized by impact on user experience.
---
...
```javascript
// ❌ BAD: 3+ second waterfall
async function initMap() {
  // Wait 1: Load mapbox-gl
  await import('mapbox-gl');

  // Wait 2: Create map
  const map = new mapboxgl.Map({ container: 'map' });

  // Wait 3: Load data after map loads
  map.on('load', async () => {
...
**Timeline:** Library load (1s) → Map init (0.5s) → Data fetch (1s) = **2.5s total**

### ✅ Solution: Parallel Loading

```javascript
// ✅ GOOD: 1 second (fastest resource wins)
async function initMap() {
  ...
  // Add data immediately when map loads
  map.on('load', () => {
    map.addSource('data', { type: 'geojson', data: mapData });
    map.addLayer({
      id: 'data-layer',
      type: 'circle',
      source: 'data'
...
**Timeline:** Max(library load, data fetch) = **~1s total**
...
### Mapbox GL JS Bundle Size

**Full bundle:** ~500 KB minified + gzipped

### ✅ Solution 1: Dynamic Imports

```javascript
// ❌ BAD: Imports mapbox-gl in main bundle
import mapboxgl from 'mapbox-gl';

// ✅ GOOD: Load mapbox-gl only when needed
...
**Savings:** 500 KB deferred from initial bundle

### ✅ Solution 2: Split by Route/Feature
...
function App() {
  return (
    <Routes>
      ...
      {/* Map loaded only when route accessed */}
      <Route
        path="/map"
        element={
...
```javascript
// ❌ BAD: All plugins in main bundle
import MapboxDraw from '@mapbox/mapbox-gl-draw';
import MapboxGeocoder from '@mapbox/mapbox-gl-geocoder';

// ✅ GOOD: Load plugins dynamically
...
// Only load when user clicks "Search" button
async function addGeocoder(map) {
  const { default: MapboxGeocoder } = await import(
    '@mapbox/mapbox-gl-geocoder'
  );
  map.addControl(new MapboxGeocoder({ accessToken: TOKEN }));
}
```

**Savings:** 100-200 KB per plugin deferred
...
- **100-1,000 markers**: Use Canvas markers or simple symbols
- **1,000-10,000 markers**: Clustering required
- **> 10,000 markers**: Server-side clustering + vector tiles
...
```javascript
// ✅ Server generates tiles, client loads only visible area
